...
const AuthPage = () => {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Cấu hình sẵn NextAuth sử dụng Credentials Provider với Username
      ...
      if (res?.error) {
        ...
        // Đăng nhập thành công, chuyển hướng về trang thông tin khách hàng
        router.push("/profile");
      }
    } catch (err) {
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Cấu hình sẵn hàm fetch gọi lên API Route Backend của bạn
      ...
      if (!res.ok) {
        ...
        // Reset form và tự động chuyển sang giao diện đăng nhập
        setFullName("");
        setPhone("");
        setIsLogin(true);
      }
  ...
  return (
    <div className="min-h-screen bg-[#FDF8F4] flex items-center justify-center p-4">
      {/* Khung chứa màu trắng với bo góc lớn và bóng đổ mịn bao quanh */}
          ...
          <>
            ...
            <form onSubmit={handleLoginSubmit} className="w-full flex flex-col gap-4">
              <input
                ...
                disabled={loading}
              />
              {/* Ô mật khẩu có viền đen dày hơn giống hệt trong ảnh */}
              <input
                type="password"
          ...
          <>
            ...
            <form onSubmit={handleRegisterSubmit} className="w-full flex flex-col gap-4">
              ...
              <input
                ...
                disabled={loading}
              />
              {/* Ô mật khẩu đăng ký cũng có viền đen dày */}
              <input
                type="password"
                placeholder="Mật khẩu *"
                value={password}
